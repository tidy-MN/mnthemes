...
md <- "| COLOR_NAME | HEX_CODE | COLOR |\n|------------|----------|-------|\n"

for (i in seq_along(mn_colors)) {
  name <- names(mn_colors)[i]
  hex  <- mn_colors[i]
  swatch <- sprintf("<span style='display:inline-block;width:40px;height:20px;background-color:%s;border:1px solid #000;'></span>", hex)
  md <- paste0(md, "| ", name, " | ", hex, " | ", swatch, " |\n")
}
